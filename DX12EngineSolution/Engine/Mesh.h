...
struct BoneInfo
{
	wstring					boneName;
	int32					parentIdx;
	Matrix					matOffset;//모델좌표계에서 해당 뼈의 좌표계로 변환해주는 행렬
};
...
private:
	...
	//FBX로더에서 AnimationData, 즉 뼈들의 FinalMatrix의 배열을 keyframe별로 SRT속성으로 
	//바꾸어 들고있게 해주는 함수이다. 
...
public:
	...
	{ return _frameBuffer[index]; } // 전체 본 프레임 정보
...
private:
	...
	vector<shared_ptr<StructuredBuffer>> _frameBuffer; // 전체 본 프레임 정보(ToRoot행렬)
};
